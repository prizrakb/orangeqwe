namespace DW.ELA.Plugin.OrangeJuice
{
    ...
    using DW.ELA.Plugin.OrangeJuice.Model;
    using DW.ELA.Utility;
    using MoreLinq;
    using NLog;
    using NLog.Fluent;

    public class OrangeJuicePlugin : AbstractBatchSendPlugin<ApiInputEvent, OrangeJuiceSettings>, IApiKeyValidator
    {
        public const string CPluginName = "OrangeJuice";
        public const string CPluginId = "OrangeJuiceUploader";
        private const string OrangeJuiceApiUrl = "https://inara.cz/inapi/v1/";
        ...
        public OrangeJuicePlugin(IPlayerStateHistoryRecorder playerStateRecorder, ISettingsProvider settingsProvider, IRestClientFactory restClientFactory, IUserNotificationInterface notificationInterface)
            : base(settingsProvider)
        {
            RestClient = restClientFactory.CreateRestClient(OrangeJuiceApiUrl);
            this.playerStateRecorder = playerStateRecorder;
            this.notificationInterface = notificationInterface;
            EventConverter = new OrangeJuiceEventConverter(this.playerStateRecorder);
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        private void SaveSettings(GlobalSettings settings, IReadOnlyDictionary<string, string> values) =>
            new PluginSettingsFacade<OrangeJuiceSettings>(PluginId).SetPluginSettings(settings, new OrangeJuiceSettings() { ApiKeys = values.ToDictionary() });
        ...
        public override async void FlushEvents(ICollection<ApiInputEvent> events)
        {
            try
            {
                ...
                {
                    var facade = new OrangeJuiceApiFacade(RestClient, commander.Name, apiKey, commander.FrontierID);
                ...
                else
                {
                    Log.Info()
                        .LoggerName(Log.Name)
                        .Message("No OrangeJuice API key set for commander, events discarded")
                        .Property("eventsCount", events.Count)
            ...
            {
                Log.Error(e, "Error while processing events for OrangeJuice");
            }
        }
        ...
        public async Task<bool> ValidateKeyAsync(string cmdrName, string apiKey)
        {
            try
            {
                var apiFacade = new OrangeJuiceApiFacade(RestClient, cmdrName, apiKey);
